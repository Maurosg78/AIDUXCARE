...
// Importación local para corregir los problemas de namespace
interface Visit {
  id: string;
  patientId: string;
...
// Tipos parciales para actualizaciones
type VisitData = Partial<Visit>;
type ClinicalEvaluationData = Partial<ClinicalEvaluation>;

interface VisitDetailPageProps {
  visitService: {
    getVisit(id: string): Promise<Visit>;
    updateVisit(id: string, data: VisitData): Promise<Visit>;
  };
  evaluationService: {
    getEvaluation(visitId: string): Promise<ClinicalEvaluation>;
    updateEvaluation(visitId: string, data: ClinicalEvaluationData): Promise<ClinicalEvaluation>;
  };
}
...
const VisitDetailPage: React.FC<VisitDetailPageProps> = ({ visitService, evaluationService }) => {
  ...
  const handleSaveEvaluation = async (data: ClinicalEvaluationData): Promise<void> => {
    if (!visitId) return;
    try {
      setLoading(true);
